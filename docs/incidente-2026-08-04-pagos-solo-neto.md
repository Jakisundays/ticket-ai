...
## 10. Actualización 2026-08-04 (posterior): `iva_alicuota` se perdía en reprocesamientos

La OP huérfana `00001-00035017` (comprobante `FAC A 0007-00009810`, proveedor SHOW IMPORT S.A.) llevó a investigar por qué esa factura tenía `iva_alicuota=0` en PocketBase pese al fix de la sección 9 ya estar desplegado. Evidencia real (no supuesta):

- El histórico de Google Sheets (`guardar_factura_completa_en_sheets`, que solo agrega filas, nunca pisa) tiene **4 corridas de extracción reales** para este comprobante, y las 4 muestran el mismo texto: `Tipo: IVA`, `Alícuota: 21.00%` -- **Gemini sí detectó el IVA correctamente**. El problema nunca fue la extracción.
- PocketBase (`invoices`) solo tiene **1 registro final** para este `process_id` (upsert por `process_id`, siempre pisa). Es decir: hubo varias corridas completas (reintentos), pero solo la ÚLTIMA en escribir determina el estado final.
- Prueba real y directa contra un record descartable de PocketBase: mandar `iva_alicuota=None` explícito se guarda como **`0`** -- PocketBase no distingue "no sé" de "es cero" en un campo `number`.
- `routes/process_invoice_google_2.py` mandaba `"iva_alicuota": _alicuota_iva` **sin condición**, en cada corrida -- a diferencia de `drive_file_id`, que sí tenía una guarda explícita para no pisar un valor bueno en un reprocesamiento.

**Causa raíz:** de las varias corridas para este comprobante, alguna extrajo el IVA bien (guardado en Sheets) y alguna no (extracción ambigua de la herramienta `impuestos_y_retenciones_de_la_factura` en ESA corrida puntual, sin relación con las demás). Como `iva_alicuota` se mandaba siempre, sin guarda, la última corrida en escribir -- sin importar si fue mejor o peor que las anteriores -- determinó el valor final. Cuando esa última corrida no pudo determinar la alícuota, `None` se convirtió en `0` (por la coerción de PocketBase) y pisó un `21.0` bueno ya guardado.

**Fix aplicado** (`_extraer_alicuota_iva` y los dos call sites de `upsert_invoice` en `worker()` y `_procesar_imagen_o_pdf_impl`, `routes/process_invoice_google_2.py`):
- `_extraer_alicuota_iva` no cambió su lógica de decisión (ya era correcta: solo devuelve un float, incluido `0.0`, cuando hay exactamente un impuesto `tipo="IVA"` con alícuota; devuelve `None` en cualquier otro caso). Se le agregó un log (`app_logger.info`, prefijo `_extraer_alicuota_iva:`) en las CUATRO ramas que devuelven `None` (antes solo una logueaba) -- así se puede ver el patrón en producción sin consultar la base de datos.
- Los call sites ahora arman los campos extraídos (`numero_comprobante`, `fecha_emision`, `tipo_comprobante`, `subtipo_comprobante`, `moneda`, `emisor_nombre`, `emisor_cuit`, `receptor_nombre`, `receptor_cuit`, `subtotal`, `total`, `cae`, `cae_vencimiento`, `forma_pago`, `iva_alicuota`, y `drive_file_id` en el segundo) en un dict aparte y **omiten cualquier campo cuyo valor sea `None`** del payload final, en vez de mandarlo explícito. Como `upsert_invoice` hace un PATCH (confirmado: `PocketBaseClient._update` usa el verbo HTTP `PATCH`, no `PUT`), un campo ausente del body deja el valor existente intacto -- PocketBase nunca lo toca. Mismo criterio que ya existía para `drive_file_id`, generalizado a todos los campos extraídos.
- Resultado: extracción válida (incluido `0.0` explícito) → se actualiza. Extracción ambigua (`None`) → se conserva lo que ya había. `0` en la base ahora solo puede significar "se detectó explícitamente 0% (o factura sin IVA)", nunca "no se pudo determinar".
- Verificado real contra un record descartable de PocketBase (creado y borrado en la misma prueba): valor bueno (21.0) + reintento con el campo omitido → se conserva 21.0; upsert posterior con `0.0` explícito → sí pisa. Sin este fix, el mismo escenario pisaba con `0`.
