...
/** Soft-delete de una Orden de Pago (ver Invoicy DELETE
 * /gemini2/payment-orders/{process_id}). Si status="success" el backend
 * exige `reason` no vacío (400 si falta) -- esta ruta solo reenvía lo que
 * mandó el cliente, la validación autoritativa vive en Invoicy, nunca acá. */
export async function DELETE(
  ...
  try {
    upstream = await fetch(
      `${INVOICE_API_BAS_URL}/gemini2/payment-orders/${encodeURIComponent(processId)}`,
      {
        method: "DELETE",
        headers: { "Content-Type": "application/json", "X-Invoicy-Secret": SECRET_KEY },
